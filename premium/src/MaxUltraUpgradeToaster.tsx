...
export const MaxUltraUpgradeToaster: React.FC<ToasterProps> = ({
  isOpen,
  onDismiss,
  onUpgrade,
}) => (
  <AnimatePresence>
    {isOpen && (
      <motion.div
        ...
        className="fixed top-4 right-4 z-50 w-80 p-4 rounded-xl border border-amber-500/30 bg-amber-500/[0.08] backdrop-blur-xl shadow-2xl"
      >
        <button
          onClick={() => onDismiss('max_ultra_upgrade')}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-white/90 text-sm font-semibold mb-1">
              Go Ultra for Maximum Power
            </h4>
            <p className="text-white/50 text-xs mb-3 leading-relaxed">
              Natively API access, priority AI, calendar integration, email follow-ups, and team collaboration.
            </p>
            <button
              onClick={() => { onDismiss('max_ultra_upgrade'); onUpgrade(); }}
            ...
            >
              Upgrade to Ultra <ArrowRight size={11} />
            </button>
          </div>
        </div>
      </motion.div>
