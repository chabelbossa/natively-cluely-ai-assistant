...
import type { MaskedKeyInfo } from "./services/CredentialsManager";
...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  safeHandle("get-provider-masked-keys", async (_, provider: string) => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      const { StoredLlmProvider } = require("./services/CredentialsManager");
      if (
        ![
          "gemini",
          "groq",
      ...
      const cm = CredentialsManager.getInstance();
      const keys = cm.getMaskedApiKeys(provider as any);
      const count = cm.getApiKeyCount(provider as any);
      return { success: true, keys, count };
    } catch (error: any) {
      console.error("Error getting masked keys:", error);
      return { success: false, error: error.message };
    }
  });

  safeHandle("add-provider-key", async (_, provider: string, key: string) => {
    try {
      const { CredentialsManager } = require("./services/CredentialsManager");
      if (
        ![
      ...
      const cm = CredentialsManager.getInstance();
      const index = cm.addApiKey(provider as any, key);
      const count = cm.getApiKeyCount(provider as any);

      const llmHelper = appState.processingHelper.getLLMHelper();
      const allKeys = (cm as any).getApiKeysList(provider).join("\n");
      if (provider === "gemini") llmHelper.setGeminiApiKey(allKeys);
      else if (provider === "groq") llmHelper.setGroqApiKey(allKeys);
      else if (provider === "deepinfra") llmHelper.setDeepInfraApiKey(allKeys);
      else if (provider === "opencode_go")
        llmHelper.setOpenCodeGoApiKey(allKeys);
      else if (provider === "openai") llmHelper.setOpenaiApiKey(allKeys);
      else if (provider === "claude") llmHelper.setClaudeApiKey(allKeys);
  ...
  safeHandle(
    "remove-provider-key",
    async (_, provider: string, index: number) => {
      try {
        const { CredentialsManager } = require("./services/CredentialsManager");
        if (
          ![
        ...
        const cm = CredentialsManager.getInstance();
        const removed = cm.removeApiKey(provider as any, index);
        const count = cm.getApiKeyCount(provider as any);
        ...
        if (removed) {
          ...
          if (provider === "gemini") llmHelper.setGeminiApiKey(allKeys);
            ...
            llmHelper.setDeepInfraApiKey(allKeys);
          else if (provider === "opencode_go")
            llmHelper.setOpenCodeGoApiKey(allKeys);
          else if (provider === "openai") llmHelper.setOpenaiApiKey(allKeys);
          else if (provider === "claude") llmHelper.setClaudeApiKey(allKeys);
  ...
  safeHandle("get-stored-credentials", async () => {
    try {
      ...
      const localSttConfig =
        CredentialsManager.getInstance().getLocalSttConfig();
      ...
      const providerKeys: Record<
        string,
        { keys: MaskedKeyInfo[]; count: number }
      > = {};
      for (const p of [
        "gemini",
        "groq",
        ...
        providerKeys[p] = {
          keys: cm.getMaskedApiKeys(p),
          count: cm.getApiKeyCount(p),
        };
      }
      ...
      return {
        ...
        localSttWhisperCppExecutablePath:
          localSttConfig.whisperCppExecutablePath,
      ...
      return {
        ...
        providerKeys: {
          gemini: { keys: [], count: 0 },
          groq: { keys: [], count: 0 },
          deepinfra: { keys: [], count: 0 },
          opencode_go: { keys: [], count: 0 },
          openai: { keys: [], count: 0 },
          claude: { keys: [], count: 0 },
        },
        ...
        localSttGlossary:
          "Kyntia, SSO, Next.js, NestJS, WaChap, Kloo, Artiweb, API, frontend, backend",
        localSttWhisperCppModelPath: path.join(
          app.getPath("home"),
          "Library/Application Support/com.prakashjoshipax.VoiceInk/WhisperModels/ggml-large-v3-turbo-q5_0.bin",
        ),
        localSttWhisperCppExecutablePath: "/opt/homebrew/bin/whisper-cli",
        hasSttGroqKey: false,
  ...
  safeHandle(
    "set-local-stt-config",
    async (
      _,
      config: {
        mode?: "server" | "whisper_cpp" | "parakeet_stream";
        endpoint?: string;
        model?: string;
        glossary?: string;
    ...
    async (
      ...
      try {
        ...
        if (localConfig.mode === "whisper_cpp") {
          if (
            !LocalSTT.isExecutableFile(localConfig.whisperCppExecutablePath)
          ) {
            return {
              success: false,
              error: `whisper-cli not found: ${localConfig.whisperCppExecutablePath || "(empty)"}`,
            };
          }
          if (!LocalSTT.isReadableFile(localConfig.whisperCppModelPath)) {
            return {
              success: false,
              error: `Whisper model not found: ${localConfig.whisperCppModelPath || "(empty)"}`,
            };
          }
          ...
          const tempPath = path.join(
            os.tmpdir(),
            `natively-local-stt-test-${Date.now()}.wav`,
          );
          ...
          try {
            await new Promise<void>((resolve, reject) => {
              execFile(
                ...
                [
                  "-m",
                  localConfig.whisperCppModelPath,
                  "-f",
                  tempPath,
                  "-nt",
                  "-np",
      ...
      try {
        ...
        if (!apiKey || !apiKey.trim()) {
          ...
          if (provider === "gemini") apiKey = creds.getGeminiApiKey();
            ...
            apiKey = creds.getDeepInfraApiKey();
          else if (provider === "opencode_go")
            apiKey = creds.getOpenCodeGoApiKey();
  ...
  safeHandle(
    ...
    async (
      _,
      feedback: { decisionId: string; rating: string; mode?: string },
    ) => {
      try {
    ...
    try {
      ...
      if (tavilyApiKey) {
        ...
        if (nativelyKey) {
          ...
          const trialToken =
            nativelyKey === "__trial__" ? cm.getTrialToken() : undefined;
