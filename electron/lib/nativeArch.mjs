 ...
 *     2. npm run rebuild:native (rebuild step)
 *     3. scripts/verify-native-arch.js (postinstall + patch preinstall guards)
 *
 *   Centralizing means one edit covers all consumers — drift between copies is
 *   exactly how a poisoned `.node` passes one gate but fails at dlopen.
 *
 * Why .mjs (not .ts):
 *   scripts/* are plain Node CommonJS that can `require()` an .mjs via
 *   dynamic import, and the boot gate uses a byte-equivalent .cjs shim
 *   (electron/lib/nativeArch.cjs) for synchronous top-level use. Keeping this
 *   file .mjs avoids coupling the scripts to the TS compile step.
 */
 ...
 *
 * Behavior:
 *   - Other platforms than darwin/win32: returns `{ ok: true, skipped: true }`.
 ...
 *   single-arg form (`verifyAll(repoRoot)`) stays in use by
 *   `scripts/verify-native-arch.js`.
 *
